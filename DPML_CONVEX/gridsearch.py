...
from algorithms.psgd import PrivateConvexPSGDLR, PrivateConvexPSGDSVM, PrivateStronglyConvexPSGDLR, \
    PrivateStronglyConvexPSGDSVM
...
def main():
    ...
    accfile = open(os.path.join(result_location, alg_name + '_' + dataset_name + times + '.acc'), 'w')
    stdfile = open(os.path.join(result_location, alg_name + '_' + dataset_name + times + '.std'), 'w')
    logfile = open(os.path.join(result_location, alg_name + '_' + dataset_name + times + '.log'), 'w')
    ...
    algorithm_names = ['AMP', 'AMP-NT','PPSSGD']
    algorithms = {
        'AMP-NT': {
                ...
                for time in range(repeat_time):
                    if dataset_name in multivariate_datasets:
                        ...
                        for i, binary_train_y in enumerate(train_ys):
                            args = [datasets_l[alg_name][hyper['L']]['training'], binary_train_y,
                                    eps / training_labels.shape[1], delta / training_labels.shape[1], hyper, model_name,
                                    counter, total_configurations]
                            result.append(pool.apply_async(alg, args))
                    else:
                        args = [datasets_l[alg_name][hyper['L']]['training'], training_labels, eps, delta, hyper,
                                model_name, counter, total_configurations]
                        result.append(pool.apply_async(alg, args))
        end_time = t.perf_counter()

        #results = np.array([res.get() for res in result])

        results = np.array([res.get() for res in result], dtype=object)
