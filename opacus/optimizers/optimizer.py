...
class DPOptimizer(Optimizer):
    def __init__(
        ...
        *,
        ...
        self.k = k # get our initial epsilon and k, which will be used to calculate the noise
    ...
    def clip_and_accumulate(self):
        # If the gradient sample is empty, initialize a zero tensor as the trimming factor
        if len(self.grad_samples[0]) == 0:
            ...
            # Calculate the gradient paradigm for each parameter
            per_param_norms = [
                g.reshape(len(g), -1).norm(2, dim=-1) for g in self.grad_samples
            ]
            # Calculate the l2 norm for each sample
            per_sample_norms = torch.stack(per_param_norms, dim=1).norm(2, dim=1)
            # Calculate the clipping factor for each sample
            per_sample_clip_factor = (
                    self.max_grad_norm / (per_sample_norms + 1e-6)
            ).clamp(max=1.0)

        for p in self.params:
            _check_processed_flag(p.grad_sample) # check if grad_sample has been used before
            grad_sample = self._get_flat_grad_sample(p) # get per sample grads and reshape
            grad = torch.einsum("i,i...", per_sample_clip_factor, grad_sample)

            if p.summed_grad is not None: # if grad has been accumulated before, add to it
                p.summed_grad += grad
            else:
                p.summed_grad = grad
    ...
    def compute_noise_and_size(self):
        shape_list = []
        for p in self.params:
            ...
            shape_list.append(noise)
        return shape_list

    def add_noise(self):
        # Compute noise list and total size
        shape_list = self.compute_noise_and_size()
        k = self.k
        ...
        # Calculate the overall magnitude of all tensors
        magnitude = torch.sqrt(sum(torch.sum(noise ** 2) for noise in shape_list))
        ...
        z = torch.abs(torch.normal(mean=0, std=1, size=(1,)))

        spherical_noise = [torch.sqrt(torch.tensor(sigma_squared)) * z * item for item in h]

        gaussian_noise_list = [
            torch.normal(mean=0.0, std=multiplier * max_grad_norm, size=noise.shape, device=noise.device)
            for noise in spherical_noise
        ]

        x = torch.sqrt(4 * torch.tensor(torch.exp(torch.tensor(1.0))) * torch.log(torch.tensor(1.25 * 60000)))

        noise_np = [item / x for item in gaussian_noise_list]

        sigma_m = [item / z for item in noise_np]





        for p , noise in zip(param_list, spherical_noise):
