...
from examples.dcgan import epsilon
...
class DPOptimizer(Optimizer):
    ...
    def _init_(
        ...
        *,
        ...
        self.preset_constant = preset_constant
    ...
    def compute_noise_and_size(self):
        ...
        for p in self.params:
            ...
            tensor_size = torch.numel(p.summed_grad)
            total_size += tensor_size
            layer_count += 1

        return noise_list, total_size, layer_count


    def add_noise(self):

        # Compute noise list and total size
        noise_list, total_size, layer_count = self.compute_noise_and_size()

        sensitivity = (np.sqrt(self.max_grad_norm ** 2 * layer_count))

        grad_samples = self.grad_samples
        sum_squared = torch.sum(torch.stack(grad_samples), dim=0)

        sum_norm_squared = torch.sum(torch.stack(grad_samples) ** 2, dim=0)

        h = sum_squared / sum_norm_squared

        k = 1000
        M = total_size
        t_squared = 2 * k ** ( 4/M ) (( M/4 + 3/2 ) **( 1 + 4 / M) / (torch.e ** (1 + 2 / M)))

        sigma_squared = (sensitivity ** 2 / epsilon ** 2 ) * t_squared


        z = torch.abs(torch.normal(mean=0, std=1, size=(1,)))
        r1smg_noise = [torch.sqrt(torch.tensor(sigma_squared)) * z * h]
        for p, noise in zip(self.params, r1smg_noise):
            _check_processed_flag(p.summed_grad)
            if p.summed_grad is not None:
